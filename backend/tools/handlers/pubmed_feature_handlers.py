"""
Handler for PubMed feature extraction using the enrichment pattern.

This handler follows the same pattern as Google Scholar, enriching articles
with features in their metadata rather than creating separate extraction objects.
"""
...
async def handle_pubmed_extract_features_enriched(input: ToolHandlerInput) -> ToolHandlerResult:
    """
    Extract research features from PubMed articles using the enrichment pattern.
    
    This handler enriches existing articles with features in their metadata,
    following the same pattern as Google Scholar feature extraction.
    
    Args:
        input: ToolHandlerInput containing:
            - articles: List of PubMed articles to analyze
...
register_tool_handler(
    "pubmed_extract_features_enriched",
    ToolExecutionHandler(
        handler=handle_pubmed_extract_features_enriched,
        description="Extracts research features from PubMed articles and enriches them with metadata"
    )
)