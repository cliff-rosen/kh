...
class InvitationService:
    ...
    async def async_list_invitations(
        ...
        """List invitations with optional filters (async)."""
    ...
    async def async_get_pending_invitation_for_email(
        self,
        email: str
    ) -> Optional[Invitation]:
        """Check if a pending invitation exists for an email (async)."""
        result = await self.db.execute(
    ...
    async def async_create_invitation(
        ...
        existing_invite = await self.async_get_pending_invitation_for_email(email)
        if existing_invite:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="A pending invitation for this email already exists"
    ...
    async def async_revoke_invitation(self, invitation_id: int) -> bool:
        """Revoke an invitation (async)."""
        result = await self.db.execute(
    ...
    async def async_get_invitation_by_id(
        self,
        invitation_id: int
    ) -> Optional[Invitation]:
        """Get an invitation by ID (async)."""
        result = await self.db.execute(
            select(Invitation).where(Invitation.invitation_id == invitation_id)
        )
        return result.scalars().first()
