...
export const adminApi = {
  ...
  /**
   * Get approved reports for scheduling dropdown (platform admin only)
   */
  async getApprovedReportsForEmail(): Promise<ApprovedReportInfo[]> {
    const response = await api.get('/api/admin/email-queue/approved-reports');
    return response.data;
  },

  /**
   * Get subscribers for a report's stream (platform admin only)
   */
  async getReportSubscribers(reportId: number): Promise<SubscriberInfo[]> {
    const response = await api.get(`/api/admin/email-queue/subscribers/${reportId}`);
    return response.data;
  },

  /**
   * Schedule emails for multiple users (platform admin only)
   */
  async scheduleEmails(data: BulkScheduleRequest): Promise<BulkScheduleResponse> {
    const response = await api.post('/api/admin/email-queue/schedule', data);
    return response.data;
  },

  /**
   * Cancel a scheduled email (platform admin only)
   */
  async cancelEmail(entryId: number): Promise<void> {
    await api.delete(`/api/admin/email-queue/${entryId}`);
  }
};
